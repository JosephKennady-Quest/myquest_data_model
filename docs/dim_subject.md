...
| `lesson_ids` | `quest_rearch_production.lessons` alias `l` (via `lagg` subquery) | `id`, `name` | `JSON_ARRAYAGG(JSON_OBJECT('id', l.id, 'name', l.name))`, computed per-subject in a `LEFT JOIN` subquery aliased `lagg`, filtered to `status = 1 AND deleted_at IS NULL` | Provides the id + name of every active, non-deleted lesson belonging to the subject, e.g. `[{"id": 42, "name": "Intro to Welding"}, ...]`. | Yes — `lagg` is `LEFT JOIN`ed, so subjects with no active lessons show `NULL`. |
| `active` | `quest_rearch_production.subjects` alias `s` | `status` | Direct select: `s.status AS active` | Indicates whether the subject is currently active. | Depends on source schema |
...
erDiagram
    ...
    subjects ||--o{ lessons : "LEFT JOIN (via subquery) on subject_id"
```
...
| `subjects` to the `lagg` (lessons) subquery | `LEFT JOIN` | Preserves subjects with no active lessons; `lesson_ids` is `NULL` in that case. Deduplication happens inside the subquery via `SELECT DISTINCT` since MySQL disallows `DISTINCT` inside `JSON_ARRAYAGG`. |
...
| Output DWH columns | `subject_id`, `subject_name`, `identity_tag`, `program_ids`, `trade_ids`, `project_ids`, `lesson_ids`, `active`, `updated_at` |
